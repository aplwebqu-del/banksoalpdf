...
export class BankSoalService {
  ...
  public async restoreBankSoal(id: string, user: User): Promise<boolean> {
    const success = this.sheetsService.restoreBankSoal(id, user);
    if (success) {
      try {
        await this.appsScriptGateway.executePostAction('restoreBankSoal', { id, user });
      } catch (err) {
        console.warn('Apps Script restore notice:', err);
      }
    }
    return success;
  }
  ...
  /**
   * Hapus Permanen Bank Soal dari Google Sheets dan Google Drive
   */
  public async permanentDeleteBankSoal(id: string, user: User): Promise<boolean> {
    const result = this.sheetsService.permanentDeleteBankSoal(id, user);
    if (result.success) {
      if (result.file_id) {
        this.driveService.deleteFile(result.file_id);
      }
      try {
        await this.appsScriptGateway.executePostAction('permanentDeleteBankSoal', { id, user });
      } catch (err) {
        console.warn('Apps Script permanent delete notice:', err);
      }
      return true;
    }
    return false;
  ...
  public async emptyTrash(user: User): Promise<{ count: number }> {
    ...
    if (res.fileIds && res.fileIds.length > 0) {
      res.fileIds.forEach((fId) => this.driveService.deleteFile(fId));
    }
    try {
      await this.appsScriptGateway.executePostAction('emptyTrash', { user });
    } catch (err) {
      console.warn('Apps Script empty trash notice:', err);
    }
  ...
  public async getHealthCheck(): Promise<{
    ...
    const hasAppsScript = Boolean(cfg.apps_script_url);
    ...
    if (hasAppsScript) {
      ...
      const testRes = await this.appsScriptGateway.testLiveConnection();
    ...
    return {
      ...
      google_drive: {
        status: 'ONLINE',
        root_folder_id: cfg.drive_root_folder_id || 'root',
      },
      google_apps_script: {
        status: gasStatus,
