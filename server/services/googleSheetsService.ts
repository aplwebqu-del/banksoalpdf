...
export class GoogleSheetsService {
    ...
    try {
      if (fs.existsSync(this.sheetsDbPath)) {
        ...
        (data.users || []).forEach((user: User) => {
          // Guarantee Dra. Hj. Nurhayati, M.Pd. has ADMIN role
          if (user.id === 'u-1' || user.email === 'nurhayati@sekolah.sch.id') {
            user.role = 'ADMIN';
            if (!user.password) user.password = 'admin_nurhayati';
            if (!user.password_hash) user.password_hash = 'admin_nurhayati';
          }
          this.tableUsers.set(user.id, user);
        });
  ...
  public getBankSoalList(params: FilterParams = {}, currentUserId?: string) {
    ...
    let filtered = list.filter((item) => {
      ...
      if (params.bab && item.bab !== params.bab) return false;
      if (params.topik && item.topik !== params.topik) return false;
  ...
  /**
   * Soft delete (pindahkan ke keranjang sampah)
   */
  public deleteBankSoal(id: string, user: User): boolean {
    ...
    this.logActivity({
      ...
      details: { judul: existing.judul, file_id: existing.file_id, status: 'arsip (keranjang sampah)' },
    });
   ...
   */
  public restoreBankSoal(id: string, user: User): boolean {
    const existing = this.tableBankSoal.get(id);
    if (!existing) return false;
  ...
  /**
   * Hapus Permanen dari Database & Storage
   */
  public permanentDeleteBankSoal(id: string, user: User): { success: boolean; file_id?: string; judul?: string } {
    const existing = this.tableBankSoal.get(id);
    if (!existing) return { success: false };

    if (user.role !== 'ADMIN' && existing.uploaded_by !== user.id) {
      throw new Error('Hanya Administrator atau pemilik soal yang dapat menghapus permanen bank soal ini.');
    }

    const fileId = existing.file_id;
    const judul = existing.judul;
    ...
    this.logActivity({
      ...
      bank_soal_id: id,
      file_id: fileId,
      details: { judul: judul, file_id: fileId },
    });

    this.persistDatabase();
    return { success: true, file_id: fileId, judul: judul };
  }

  /**
   * Kosongkan Semua Keranjang Sampah
   */
  public emptyTrash(user: User): { count: number; fileIds: string[] } {
    const fileIds: string[] = [];
    let count = 0;

    Array.from(this.tableBankSoal.entries()).forEach(([id, item]) => {
  ...
  public getTrashCount(): number {
    return Array.from(this.tableBankSoal.values()).filter((i) => i.status === 'arsip').length;
  }
  ...
  public getUserByEmail(email: string): User | null {
    ...
    for (const u of this.tableUsers.values()) {
  ...
  public createUser(userData: Partial<User>, adminUser?: User): User {
    const id = `u-${Date.now()}`;
    const now = new Date().toISOString();

    const newUser: User = {
      id,
      name: userData.name || 'Pengajar Baru',
      ...
      school_institution: userData.school_institution || userData.institution || 'SMA Negeri Indonesia',
      subject: userData.subject || 'Mata Pelajaran Umum',
      created_at: now,
      updated_at: now,
    };

    this.tableUsers.set(id, newUser);
  ...
  public updateUser(id: string, updates: Partial<User>, adminUser?: User): User | null {
    const existing = this.tableUsers.get(id);
    if (!existing) return null;
    ...
    this.tableUsers.set(id, updated);
  ...
  public deleteUser(id: string, adminUser?: User): boolean {
    const existing = this.tableUsers.get(id);
    if (!existing) return false;

    this.tableUsers.delete(id);

    if (adminUser) {
      this.logActivity({
        user: adminUser,
        action: 'DELETE_USER',
        details: { target_user_id: id, name: existing.name },
      });
    }
  ...
  public updateUserStatus(id: string, status: 'ACTIVE' | 'INACTIVE', adminUser?: User): User | null {
    return this.updateUser(id, { status }, adminUser);
  }
