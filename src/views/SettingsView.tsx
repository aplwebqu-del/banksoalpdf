...
export const SettingsView: React.FC = () => {
  ...
  const openAddModal = () => {
    ...
    setFormDriveId('1cYgXdF66T1GKMw3Dkd26J55E5HZPIRBG');
    setFormSheetsId('1xWK5VMJatqMtshx0FnFKZL54N2lQZ7wXoEJCQVT1o9A');
    setFormDescription('');
  ...
  return (
    <div className="space-y-8 pb-16 animate-in fade-in duration-200">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1 max-w-2xl">
            Kelola multi profil penyimpanan Google Drive & Google Sheets. Seluruh fitur (Upload, Search, Preview, Download, Edit, Delete, Sync) beroperasi pada profil <strong>Active Storage</strong> yang dipilih.
          </p>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-400 hidden sm:inline">
            Digunakan oleh seluruh operasi Upload, Search, Preview, Download & Sync
          </span>
        </div>
        ...
        {activeProfile ? (
          <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950/20 border-2 border-emerald-500/60 p-6 rounded-3xl shadow-2xl space-y-6 relative overflow-hidden">
              ...
              <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-2.5 hover:border-slate-700 transition-colors">
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-[11px] text-slate-400 flex items-center gap-2">
                    <span>Latency: <strong className="text-emerald-400 font-mono">{activeProfile.latency_ms || 320} ms</strong></span>
                  </div>
                </div>
                <div className="text-[11px] text-slate-400 truncate font-mono pt-0.5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2 text-[11px] text-slate-400 border-t border-slate-800/80">
              <div className="flex items-center gap-4">
                <span>
                  ...
                  <strong className="text-slate-200 font-mono">
                    {activeProfile.last_connection_check || activeProfile.last_connection_test
                      ? new Date(
                          activeProfile.last_connection_check || activeProfile.last_connection_test!
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleSyncDriveAndSheets(activeProfile.id)}
                  disabled={isSyncing}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold transition-all disabled:opacity-50 cursor-pointer"
                >
      ...
      {testResult && (
        <div className="p-5 rounded-3xl bg-slate-900 border border-blue-500/30 text-xs space-y-3 shadow-xl animate-in fade-in duration-150">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-[11px]">
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
              <span className="text-slate-400 block">Google Apps Script:</span>
            ...
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
              ...
              <span className={`font-semibold ${testResult.google_drive.connected ? 'text-emerald-400' : 'text-amber-400'}`}>
                {testResult.google_drive.connected ? '✓ Terhubung' : '✕ Belum Terverifikasi'} (Folder "{testResult.google_drive.folder_name}")
              </span>
            </div>
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
              <span className="text-slate-400 block">Google Sheets:</span>
              <span className={`font-semibold ${testResult.google_sheets.connected ? 'text-emerald-400' : 'text-amber-400'}`}>
                {testResult.google_sheets.connected ? '✓ Terhubung' : '✕ Belum Terverifikasi'} ({testResult.google_sheets.sheet_count || 8} Sheets)
              </span>
            </div>
          </div>
        </div>
      ...
      {/* SINKRONISASI RESULT BANNER */}
      {syncReport && (
        <div className="p-5 rounded-3xl bg-slate-900 border border-emerald-500/30 text-xs space-y-3 shadow-xl animate-in fade-in duration-150">
          <div className="flex items-center justify-between font-bold text-white">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" />
              <span>Hasil Sinkronisasi Dua Arah Google Drive ↔ Google Sheets</span>
            </div>
            <span className="text-slate-400 font-mono text-[11px]">ID: {syncReport.sync_id}</span>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px]">
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-center">
              <span className="text-slate-400 block">Total Diperiksa</span>
              <span className="text-white text-base font-bold font-mono">{syncReport.total_scanned}</span>
            </div>
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-center">
              <span className="text-slate-400 block">Disinkronkan</span>
              <span className="text-emerald-400 text-base font-bold font-mono">{syncReport.synced_count}</span>
            </div>
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-center">
              <span className="text-slate-400 block">Berkas Hilang / Missing</span>
              <span className="text-amber-400 text-base font-bold font-mono">{syncReport.missing_count}</span>
            </div>
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-center">
              <span className="text-slate-400 block">Unindexed</span>
              <span className="text-blue-400 text-base font-bold font-mono">{syncReport.unindexed_count || 0}</span>
            </div>
          </div>
          <p className="text-slate-300 text-[11px]">{syncReport.details}</p>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-500">
            Dapat diaktifkan sewaktu-waktu sebagai Active Storage
          </span>
        </div>
        ...
        {nonActiveProfiles.length === 0 ? (
          <div className="p-6 rounded-3xl bg-slate-900/60 border border-dashed border-slate-800 text-center space-y-2">
            ...
            <button
              onClick={openAddModal}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-blue-600/20 hover:bg-blue-600/30 text-blue-300 border border-blue-500/30 text-xs font-semibold transition-all cursor-pointer"
            >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase ${
                        p.connection_status === 'CONNECTED' || p.status === 'connected'
                          ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                          : p.connection_status === 'ERROR' || p.status === 'error'
                          ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20'
                          : 'bg-slate-800 text-slate-400'
                      }`}
                    >
                      {p.connection_status || p.status}
                    </span>
                  </div>
                  ...
                  <div className="space-y-1.5 text-xs text-slate-300 bg-slate-950/60 p-3 rounded-2xl border border-slate-800/80">
                    <div className="flex items-center justify-between text-[11px]">
                      ...
                      <span className="font-mono text-slate-300 truncate max-w-[180px]">
                        {p.apps_script_url ? p.apps_script_url.slice(0, 30) + '...' : '(Belum diatur)'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-slate-400">Drive Folder ID:</span>
                      <span className="font-mono text-slate-300 truncate max-w-[180px]">
                        {driveId ? driveId.slice(0, 16) + '...' : '(Belum diatur)'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-slate-400">Spreadsheet ID:</span>
                      <span className="font-mono text-slate-300 truncate max-w-[180px]">
                        {sheetId ? sheetId.slice(0, 16) + '...' : '(Belum diatur)'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border border-emerald-500/30 text-xs font-bold transition-all cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium transition-all disabled:opacity-50 cursor-pointer"
                      >
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-slate-300 bg-slate-800 hover:bg-slate-700 text-xs font-medium transition-all cursor-pointer"
                        title="Kelola Profil"
                      >
                        <Settings className="w-3.5 h-3.5" />
                        <span>Kelola</span>
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-xl text-rose-400 hover:text-rose-300 hover:bg-rose-950/40 transition-all cursor-pointer"
                        title="Hapus Profil"
                      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-800">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-4 shadow-xl">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 border border-purple-500/40 font-bold text-xs shadow-md transition-all disabled:opacity-50 cursor-pointer"
          >
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-4 shadow-xl">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl bg-blue-600 hover:bg-blue-500 font-bold text-white text-xs shadow-md shadow-blue-600/30 transition-all disabled:opacity-50 cursor-pointer"
          >
          ...
          <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
            ...
            <div className="p-6 space-y-4 text-xs">
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-white text-sm">{manageProfile.name}</span>
                  <span
                  ...
                  >
                    {manageProfile.is_active ? 'ACTIVE STORAGE' : 'INACTIVE'}
                  </span>
                </div>
                <p className="text-slate-400 text-xs">
                  {manageProfile.description || 'Tidak ada deskripsi profil.'}
                </p>
                <div className="pt-2 border-t border-slate-800 space-y-1 font-mono text-[11px] text-slate-300">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">Apps Script URL:</span>
                    <span className="truncate max-w-[220px]">{manageProfile.apps_script_url || '-'}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">Folder ID:</span>
                    <span className="truncate max-w-[220px]">
                      {extractDriveFolderId(manageProfile.drive_root_folder_id || manageProfile.google_drive_folder_id) || '-'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">Spreadsheet ID:</span>
                    <span className="truncate max-w-[220px]">
                      {extractSpreadsheetId(manageProfile.spreadsheet_id || manageProfile.google_spreadsheet_id) || '-'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2">
                ...
                {!manageProfile.is_active ? (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 rounded-2xl bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border border-emerald-500/30 font-bold transition-all cursor-pointer"
                  >
                    <div className="flex items-center gap-2.5">
                      <Power className="w-4 h-4 text-emerald-400" />
                      <span>Jadikan sebagai Active Storage</span>
                    </div>
                    <span className="text-[10px] font-normal text-emerald-400/80">Alihkan seluruh operasi ke storage ini</span>
                  </button>
                ) : (
                  <div className="p-3 rounded-2xl bg-emerald-950/40 border border-emerald-500/30 text-emerald-300 flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                    <span>Profil ini sedang <strong>Aktif</strong> sebagai Active Storage sistem.</span>
                  </div>
                )}
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between p-3 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-semibold transition-all cursor-pointer disabled:opacity-50"
                >
                  <div className="flex items-center gap-2.5">
                    <RefreshCw className={`w-4 h-4 ${testingId === manageProfile.id ? 'animate-spin text-blue-400' : 'text-blue-400'}`} />
                    <span>Uji Koneksi (Test Connectivity)</span>
                  </div>
                  <span className="text-[10px] font-normal text-slate-400">Verifikasi Drive, Sheets & Apps Script</span>
                </button>

                {/* Sinkronisasi Drive ↔ Sheets */}
                <button
                  onClick={() => handleSyncDriveAndSheets(manageProfile.id)}
                  disabled={isSyncing}
                  className="w-full flex items-center justify-between p-3 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-semibold transition-all cursor-pointer disabled:opacity-50"
                >
                  <div className="flex items-center gap-2.5">
                    <RefreshCw className={`w-4 h-4 ${isSyncing ? 'animate-spin text-purple-400' : 'text-purple-400'}`} />
                    <span>Sinkronisasi Drive ↔ Sheets</span>
                  </div>
                  <span className="text-[10px] font-normal text-slate-400">Scan & Sinkronkan metadata</span>
                </button>

                {/* Edit Profil */}
                <button
                  onClick={() => {
                    const p = manageProfile;
                    setManageProfile(null);
                    openEditModal(p);
                  }}
                  className="w-full flex items-center justify-between p-3 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-semibold transition-all cursor-pointer"
                >
                  <div className="flex items-center gap-2.5">
                    <Edit3 className="w-4 h-4 text-amber-400" />
                    <span>Edit Konfigurasi Storage</span>
                  </div>
                  <span className="text-[10px] font-normal text-slate-400">Ubah URL, ID Folder & Spreadsheet</span>
                </button>

                {/* Hapus Profil */}
                <button
                  onClick={() => handleDeleteProfile(manageProfile)}
                  disabled={profiles.length <= 1}
                  className="w-full flex items-center justify-between p-3 rounded-2xl bg-rose-950/30 hover:bg-rose-900/40 text-rose-300 border border-rose-900/40 font-semibold transition-all cursor-pointer disabled:opacity-40"
                >
                  <div className="flex items-center gap-2.5">
                    <Trash2 className="w-4 h-4 text-rose-400" />
                    <span>Hapus Profil Storage</span>
                  </div>
                  <span className="text-[10px] font-normal text-rose-400/80">
                    {profiles.length <= 1 ? 'Minimal 1 profil tersisa' : 'Hapus permanen dari sistem'}
                  </span>
                </button>
              </div>
            </div>
      ...
      {/* ADD / EDIT MODAL (+ TAMBAH GOOGLE STORAGE) */}
            ...
            <form onSubmit={handleSaveProfile} className="p-6 space-y-4 text-xs overflow-y-auto">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="Contoh: Bank Soal Utama, Bank Soal Cabang Surabaya"
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="Keterangan tujuan atau cakupan profil penyimpanan ini..."
              ...
              <div className="space-y-1.5">
                <label className="font-semibold text-slate-300 flex items-center justify-between">
                  ...
                  >
                    ...
                    <span>{showScriptGuide ? 'Tutup Petunjuk' : 'Petunjuk Setup'}</span>
                  </button>
                </label>
                <input
                  type="url"
                ...
                <p className="text-[11px] text-slate-500">
                  URL eksekusi Web App Google Apps Script sebagai API gateway ke Drive & Sheets.
                </p>
              </div>
              ...
              {showScriptGuide && (
                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 space-y-1.5 text-[11px] text-slate-400">
                  ...
                  <p>1. Buka <code>script.google.com</code> dan gunakan file <code>google-apps-script/Code.gs</code>.</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="Contoh: 1cYgXdF66T1GKMw3Dkd26J55E5HZPIRBG atau URL folder"
                  ...
                  />
                  <p className="text-[10px] text-slate-500">Mendukung input URL folder Google Drive atau ID langsung.</p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="Contoh: 1xWK5VMJatqMtshx0FnFKZL54N2lQZ7wXoEJCQVT1o9A atau URL sheet"
                    className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-white font-mono text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                  <p className="text-[10px] text-slate-500">Mendukung input URL spreadsheet atau ID langsung.</p>
                </div>
              </div>
